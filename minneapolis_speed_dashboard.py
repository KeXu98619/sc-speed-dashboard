...
Data sources (generate once with the preprocess scripts):
  ...
  python preprocess_h3_trips.py  (run by coworker once hex data is available)
"""
...
GEOMETRY_PARQUET = "data/minneapolis_geometry.parquet"
...
MINNEAPOLIS  = pdk.ViewState(latitude=44.98, longitude=-93.27, zoom=11, pitch=0)
TOOLTIP_STYLE = {
...
def load_data(period_key: str) -> tuple[pd.DataFrame, bool, list[int]]:
    ...
    metrics_df = pd.read_parquet(f"data/minneapolis_{period_key}_metrics.parquet")
    df = geom_df.merge(metrics_df, on="segmentId", how="left")

    # Merge actual AADT by segmentId (direct match for Minneapolis)
    try:
...
def render_deck(layers: list, tooltip: dict, height: int = 720) -> None:
    st.pydeck_chart(
        pdk.Deck(layers=layers, initial_view_state=MINNEAPOLIS,
...
st.set_page_config(
    page_title="Minneapolis High-Risk Link Analysis",
    layout="wide",
...
def check_password() -> bool:
    if st.session_state.get("authenticated"):
        return True
    st.title("Minneapolis High-Risk Link Analysis")
...
with ctrl:
    ...
    if show_at:
        at_min_vol = st.slider("Min bike & ped volume", min_value=0, max_value=100, value=20, step=5)
    else:
        at_min_vol = 20
    ...
    if show_h3 and len(h3_df_full) > 0:
        ...
        st.caption(
            ...
            f"(p95 ≈ {h3_df_full['daily_trips'].quantile(0.95):.0f} trips/day)"
        )
    else:
        h3_min_trips = 0
...
with main_col:
    ...
    if n_flagged == 0:
        ...
        table_df = pd.DataFrame({
            ...
            "Social Vulnerability Index (hex)": np.where(
                                                    risk_df["svi_disp"].values == "",
                                                    "—",
                                                    risk_df["svi_disp"].values,
                                                ),
            "Crash Rating (hex)":               np.where(
        ...
        st.dataframe(
            ...
            column_config={
                ...
                "Crash Rating (hex)":       st.column_config.TextColumn(),
            },
        )
