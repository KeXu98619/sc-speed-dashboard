...
with ctrl:
    ...
    t2_frc = st.multiselect(
        "Road Functional Class (FRC)", options=available_frc, default=DEFAULT_FRC,
    ...
    st.divider()
    show_at = st.checkbox("Show Bike+Ped Flows (Network) Overlay - 2025", value=False)
    if show_at:
        at_min_vol = st.slider("Min bike & ped volume", min_value=0, max_value=500, value=1, step=1)
    else:
        at_min_vol = 1

    show_h3 = st.checkbox("Show Bike+Ped Trip Activity (Hex Zones) Overlay - 2025", value=False)
...
with main_col:
    tab1, tab2 = st.tabs(["P85 Speed Exceedance", "High-Risk Links"])
    ...
    with tab1:
        ...
        ax.set_title(
            ...
            "(blue dashed = High Risk Top 1% Links,  green dashed = High Risk Top 3% Links)"
        )
        plt.tight_layout()
        st.pyplot(fig, use_container_width=True)
        plt.close(fig)

    # ── Tab 2: Risk map + table ───────────────────────────────────────────────
    with tab2:
        ...
        if n_flagged == 0:
            ...
            min_aadt = tbl_col2.slider("Minimum AADT (table filter)",
            ...
            svi_filter = tbl_col4.multiselect(
                "Social Vulnerability Index (SVI) Rating (table filter)",
                options=SVI_OPTIONS,
                default=SVI_OPTIONS,
            )
            ...
            # Build table with a hidden _pos column to trace rows back to risk_df paths.
            ...
            # Paths list aligned to current table row order (after sort + filter).
